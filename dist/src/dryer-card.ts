...
export class DryerCard extends LitElement {
  ...
  render() {
    ...
    const isOn = s["switch.dryer_power"]?.state === "on";
    ...
    const ringColor =
      state === "done" ? "#00cc66" : state === "running" ? "#f39c12" : "#888888";
      ...
      <div class="card ${!isOn ? "off" : ""}">
        ...
        <div class="tile-grid">
          ${d.includes("run_state") && e.run_state
            ? html`<div class="tile">${icon("mdi:tumble-dryer")}<div class="value">${state}</div><div class="label">Status</div></div>` : ""}
