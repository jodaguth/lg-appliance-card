 ...
 * Copyright 2017 Google LLC
 * SPDX-License-Identifier: BSD-3-Clause
 */function ht(t){return(e,i)=>"object"==typeof i?ct(t,e,i):((t,e,i)=>{const s=e.hasOwnProperty(i);return e.constructor.createProperty(i,t),s?Object.getOwnPropertyDescriptor(e,i):void 0})(t,e,i)}let pt=class extends nt{hass;config;static styles=n`
    :host {
      display: block;
    }
  ...
  `;render(){const t=this.config.entities,e=this.config.display,i=this.hass.states,s=i[t.run_state]?.state||"unknown",r="on"===i["switch.washer_power"]?.state,o=i[t.initial_time]?.state||"0:00",n=i[t.remaining_time]?.state||"0:00",[a,l]=o.split(":").map(Number),[d,c]=n.split(":").map(Number),h=60*a+l,p=60*d+c,u=h>0?Math.min(100,(h-p)/h*100):0,f="done"===s?"#00cc66":"running"===s?"#1e90ff":"#888888";this.style.setProperty("--ring-color",f),this.style.setProperty("--angle",`${(3.6*u).toFixed(1)}deg`);const v=t=>I`<ha-icon icon="${t}"></ha-icon>`;return I`
      <div class="card ${r?"":"off"}">
        ...
        <div class="ring-wrap">
          <div class="ring">
            <span>${"done"===s?"Done":`${u.toFixed(0)}%`}</span>
          </div>
        </div>

        <div class="tile-grid">
          ${e.includes("run_state")&&t.run_state?I`<div class="tile">${v("mdi:play-circle")}<div class="value">${s}</div><div class="label">Status</div></div>`:""}
          ${e.includes("spin_speed")&&t.spin_speed?I`<div class="tile">${v("mdi:rotate-right")}<div class="value">${i[t.spin_speed]?.state??"-"}</div><div class="label">Spin</div></div>`:""}
          ${e.includes("water_temp")&&t.water_temp?I`<div class="tile">${v("mdi:thermometer")}<div class="value">${i[t.water_temp]?.state??"-"}</div><div class="label">Temp</div></div>`:""}
          ${e.includes("course")&&t.course?I`<div class="tile">${v("mdi:playlist-check")}<div class="value">${i[t.course]?.state??"-"}</div><div class="label">Course</div></div>`:""}
          ${e.includes("run_completed")&&t.run_completed?I`<div class="tile">${v("mdi:check-circle")}<div class="value">${i[t.run_completed]?.state??"-"}</div><div class="label">Complete</div></div>`:""}
        </div>
      </div>
  ...
  `;render(){const t=this.config.entities,e=this.config.display,i=this.hass.states,s=i[t.run_state]?.state||"unknown",r="on"===i["switch.dryer_power"]?.state,o=i[t.initial_time]?.state||"0:00",n=i[t.remaining_time]?.state||"0:00",[a,l]=o.split(":").map(Number),[d,c]=n.split(":").map(Number),h=60*a+l,p=60*d+c,u=h>0?Math.min(100,(h-p)/h*100):0,f="done"===s?"#00cc66":"running"===s?"#1e90ff":"#888888";this.style.setProperty("--ring-color",f),this.style.setProperty("--angle",`${(3.6*u).toFixed(1)}deg`);const v=t=>I`<ha-icon icon="${t}"></ha-icon>`;return I`
      <div class="card ${r?"":"off"}">
        ${r?"":I`<div class="overlay">Off</div>`}
  ...
  `;setConfig(t){if(!t.washer?.entities&&!t.dryer?.entities)throw new Error("Washer or Dryer configuration must be provided.");this._config=t}getCardSize(){return 2}render(){if(!this._config||!this.hass)return I`<div class="error">❌ LG Appliance Card is not configured correctly.</div>`;const t=this._config.washer?.enabled&&this._config.washer.entities&&Object.values(this._config.washer.entities).some(Boolean),e=this._config.dryer?.enabled&&this._config.dryer.entities&&Object.values(this._config.dryer.entities).some(Boolean);return t||e?I`
    ...
    `:I`<div class="error">❌ LG Appliance Card: No valid washer or dryer entities defined.</div>`}};t([ht({attribute:!1})],ft.prototype,"hass",void 0),t([ht({type:Object})],ft.prototype,"_config",void 0),ft=t([lt("lg-appliance-card")],ft);export{ft as LGApplianceCard};
