 ...
 * Copyright 2017 Google LLC
 * SPDX-License-Identifier: BSD-3-Clause
 */function ht(t){return(e,i)=>"object"==typeof i?dt(t,e,i):((t,e,i)=>{const s=e.hasOwnProperty(i);return e.constructor.createProperty(i,t),s?Object.getOwnPropertyDescriptor(e,i):void 0})(t,e,i)}let pt=class extends nt{static{this.styles=n`
    :host {
      display: block;
      max-width: 300px;
      margin: auto;
  ...
  `}render(){const t=this.config.entities,e=this.config.display,i=this.hass.states,s=i[t.run_state]?.state||"unknown",r="on"===i["switch.washer_power"]?.state,o=i[t.initial_time]?.state||"0:00",n=i[t.remaining_time]?.state||"0:00",[a,l]=o.split(":").map(Number),[c,d]=n.split(":").map(Number),h=60*a+l,p=60*c+d,u=h>0?Math.min(100,(h-p)/h*100):0,f=2*Math.PI*45,m="done"===s?"#00cc66":"running"===s?"#1e90ff":"#888888";this.style.setProperty("--ring-color",m);const g=t=>I`<ha-icon icon="${t}"></ha-icon>`;return I`
      <div class="card ${r?"":"off"}">
        ...
        <div class="tile-grid">
          ${e.includes("run_state")&&t.run_state?I`<div class="tile">${g("mdi:play-circle")}<div class="value">${s}</div><div class="label">Status</div></div>`:""}
          ${e.includes("spin_speed")&&t.spin_speed?I`<div class="tile">${g("mdi:rotate-right")}<div class="value">${i[t.spin_speed]?.state??"-"}</div><div class="label">Spin</div></div>`:""}
          ${e.includes("water_temp")&&t.water_temp?I`<div class="tile">${g("mdi:thermometer")}<div class="value">${i[t.water_temp]?.state??"-"}</div><div class="label">Temp</div></div>`:""}
          ${e.includes("course")&&t.course?I`<div class="tile">${g("mdi:playlist-check")}<div class="value">${i[t.course]?.state??"-"}</div><div class="label">Course</div></div>`:""}
          ${e.includes("run_completed")&&t.run_completed?I`<div class="tile">${g("mdi:check-circle")}<div class="value">${i[t.run_completed]?.state??"-"}</div><div class="label">Complete</div></div>`:""}
        </div>
      </div>
    `}};t([ht({attribute:!1})],pt.prototype,"hass",void 0),t([ht({type:Object})],pt.prototype,"config",void 0),pt=t([lt("washer-card")],pt);let ut=class extends nt{static{this.styles=n`
    :host {
      display: block;
      max-width: 300px;
      margin: auto;
  ...
  `}render(){const t=this.config.entities,e=this.config.display,i=this.hass.states,s=i[t.run_state]?.state||"unknown",r="on"===i["switch.dryer_power"]?.state,o=i[t.initial_time]?.state||"0:00",n=i[t.remaining_time]?.state||"0:00",[a,l]=o.split(":").map(Number),[c,d]=n.split(":").map(Number),h=60*a+l,p=60*c+d,u=h>0?Math.min(100,(h-p)/h*100):0,f=2*Math.PI*45,m="done"===s?"#00cc66":"running"===s?"#f39c12":"#888888";this.style.setProperty("--ring-color",m);const g=t=>I`<ha-icon icon="${t}"></ha-icon>`;return I`
      <div class="card ${r?"":"off"}">
        ...
        <div class="tile-grid">
          ${e.includes("run_state")&&t.run_state?I`<div class="tile">${g("mdi:tumble-dryer")}<div class="value">${s}</div><div class="label">Status</div></div>`:""}
          ${e.includes("course")&&t.course?I`<div class="tile">${g("mdi:playlist-check")}<div class="value">${i[t.course]?.state??"-"}</div><div class="label">Course</div></div>`:""}
          ${e.includes("run_completed")&&t.run_completed?I`<div class="tile">${g("mdi:check-circle")}<div class="value">${i[t.run_completed]?.state??"-"}</div><div class="label">Complete</div></div>`:""}
        </div>
      </div>
    `}};t([ht({attribute:!1})],ut.prototype,"hass",void 0),t([ht({type:Object})],ut.prototype,"config",void 0),ut=t([lt("dryer-card")],ut);let ft=class extends nt{static{this.styles=n`
    :host {
      display: block;
    }
    .wrapper {
